...
{
    public class OrgLetterCategoryController : Controller
    {
        private readonly ApplicationDbContext _dbcontext;
        public OrgLetterCategoryController(ApplicationDbContext context)
        {
            _dbcontext = context;
        }
        ...
        public IActionResult Create()
        {
            ...
            ViewBag.MaxSrNo = _dbcontext.OrgLetterCategoryMaster.Where(s => s.DeleteFlag == "N" && s.OrgID == OrgID).Max(p => p.SrNo) + 1;
            //ViewBag.RTypeList = new SelectList(GetRTypeList(), nameof(RType.RTypeID), nameof(RType.RTypeName));
            return View();
        }

        [HttpPost]
        public IActionResult Create(OrgLetterCategory model)
        {
        ...
        public IActionResult Edit(Int64? id)
        {
            ...
            var data = _dbcontext.OrgLetterCategoryMaster.Find(id);
            if (data == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        public IActionResult Edit(OrgLetterCategory model)
        {
            if (ModelState.IsValid)
            {
                model.OrgID = Convert.ToInt32(HttpContext.Session.GetString("OrgID"));
        ...
        public JsonResult DisplayRecord()
        {
            ...
            {
                var data = _dbcontext.OrgLetterCategoryMaster.OrderBy(sortColumn + " " + sortColumnDirection).AsNoTracking().ToList();
                ...
                {
                    data = data.Where(x =>
                      x.LetterCategoryID.ToString() != null && x.LetterCategoryID.ToString().ToLower().Contains(searchValue.ToLower())
                      || x.LetterCategoryName != null && x.LetterCategoryName.ToLower().Contains(searchValue.ToLower())
                    ).ToList();
                }
